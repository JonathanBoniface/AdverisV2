...
class CreateWorkersFromXMLCommand extends Command
{
    /**
    ...
    public function __construct(EntityManagerInterface $em, string $dataDirectory, WorkersRepository $workersRepository )
    {
        parent::__construct();
    ...
    protected static $defaultName = 'CreateWorkersFromXML';
    ...
    private function createWorkers(): void
    {
        ...
        foreach ($this->getDataFromFile() as $row){
            if (is_array($row) && array_key_exists('workers', $row) && !empty($row['workers'])) {
                $worker = $this->workersRepository->findOneBy([
                    'name' => $row['workers']
                ]);
                if(!$worker){
                    $worker = new Workers();

                    $worker->setName($row['workers']);
